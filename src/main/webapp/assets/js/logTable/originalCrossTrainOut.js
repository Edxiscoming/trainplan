            	...
            	{    var train = data.trainList[i];
                     ...
                     if(train.commonlineRule===null){
                    	 if(train.appointWeek!==null){
                        	 ...
                        	 $("input[name='apointDaySolid']").each(function(index) {
                            ...
                            $("#stopDays").val("");
                        	 ...
                        	 if(train.appointDay!==null){
                            	 ...
                            	 if(appointDay.indexOf(",")>0){
                            		 ...
                            		 if(days[0].length===1){
                            			 $("input[target='#monthTab']").prop("checked",true); 
                                    	 ...
                                    	 for(i=0;i<days.length;i++){
                                    		 var key=days[i];
                                    		 appointDayShow += ", "+key.substr(0,4)+"年"+key.substr(4,2)+"月"+key.substr(6,2)+"日";
                                    	 }
                                		 $("#J_input").val(appointDayShow.substr(2));
                            		 }
...
//	}
	...
	if((document.getElementById("createCrossFlag")!=1)){
		alertMessage("warning","已经生成开行交路，请联系但当局。");
		return false;
	}
	var cross ={
...
//	
	...
	switch(runType){
		...
		case "weekTab" : 
			...
			break;
		case "dateTab" : 
			var operationValue = $("#J_input").val();
			if(operationValue===""){
				...
				while(/[\u4E00-\u9FA5]+/.test(operationValue)){
					operationValue=operationValue.replace(/[\u4E00-\u9FA5]+/,""); 
					operationValue = operationValue.replace(/\s/,""); //去掉空格
				}
			}
			if(operationValue ===""){
		...
		case "monthTab" : 
			...
			operationValue = operationValue.substr(1);
			if(operationValue ===""){
				alertMessage("warning","开行规律每月必选一天");
				return;
			}
			train.appointDay = operationValue; 
			train.appointPeriod = ""; 
...
function initTrainNumber (cmPartOriginalCrossId) {
	...
	$.ajax({
		...
		success: function (json) {
			...
			$("#createCrossFlag").val(json.data.cross.createCrossFlag);
			//初始化滑块信息
...
function searchCross(obj){
	...
	if(checkFlag.length)
		...
		checkFlag='';
	var isArriOrOverFlag = $("#activeChange button.active").val();
	//等级
	...
	if(obj!==undefined){
		$("#exceptionChange button.active").removeClass("active");
		$(obj).addClass("active");
		trainType = $(obj).val();
	}else{
		trainType=$("#exceptionChange button.active").val();
	}
	var param = {
		...
		isArriOrOverFlag:isArriOrOverFlag
	};
...
function searchCross2(obj){
	...
	var trainNbr = $("#trainNbr").val();
	var checkFlag=$("#checkFlag:checked");
	var trainType = $("#exceptionChange button.active").val();
	if(checkFlag.length)
		checkFlag=1;
	else
		checkFlag='';
	...
	if(obj!==undefined){
		$("#activeChange button.active").removeClass("active");
		$(obj).addClass("active");
		isArriOrOverFlag = $(obj).val();
	}else{
		trainType=$("#activeChange button.active").val();
	}
	var param = {
		cmVersionId	:cmVersionId,
		tokenVehBureau : $("#tokenBureau").val(),
	       		        ...
	       		        if(html===""){
	       		        	...
	       		        	if(html===""){
	       		        		...
	       		        		if(html===""){
	       		        			...
	       		        			var appointPeriod = row.appointPeriod;
	       		        			if(/^1+0*$/.test(appointPeriod)){
                            			 var runDays=0;
                                		 var stopDays= 0;
                            			 for(var i=0;i<appointPeriod.length;i++){
 ...
 * 设置开行规律“日期”
 */
function openRegularSet(){
    $('#regularSet').show().dialog({
        title: '设置开行规律——日期',
        width: 800,
        height: 500,
        cache: false,
...
/**
 * 确定添加车次
 * confirmedType为1时时追加，0是插入
 */

function confirmedTrain(confirmedType){
	//下方被选中的
	var $DownselectedTrain=$("input[name='crossIndex']:checked").parent();
	
	//上方要添加的
	var selectedTrain = $($("#result a.active")[0]).data("trainObj");
	
	var trainList = $("#numberDetail");
	var trainObjArr=new Array();
	$("#numberDetail li").each(function(index){
		var trainObj = $(this).data("trainObj");
		trainObjArr.push(trainObj);
	});
	debugger;
	//下方选中则追加或插入，否则添加到最后一行
	if($DownselectedTrain.length>0)
	{
		
		var index=parseInt($DownselectedTrain.find(".indexOfTrain")[0].innerText)+confirmedType;
	}
	else{
	   var index = trainList.children().length+1;
	}
	var $liList = $('<li class="row"><label class="control-label pull-left"><input style="margin-right:5px;" type="radio" name="crossIndex"><span class="indexOfTrain">' + index + '</span><span class="trainNo">．'+selectedTrain.trainName+ '</span><span style="margin-left:20px;">'+selectedTrain.crossSection+'</span></label>'+
    '<div class="pull-right" style="margin-right:20px;"><button class="btn btn-default btn-sm redBtn" onclick="deleteTrainNumber(this.parentNode.parentNode);">删除</button>'+
    '<button class="btn btn-default btn-sm" style="margin-left:45px;" onclick="upRow(this.parentNode.parentNode);">上移</button>'+
    '<button class="btn btn-default btn-sm" style="margin-left:20px;" onclick="downRow(this.parentNode.parentNode);">下移</button></div></li>');
	...
	var trainListArr=trainList.children("li");
	trainList.empty();
	for(var i=0;i<index-1;i++)
	{
		trainList.append($(trainListArr[i]).data("trainObj",trainObjArr[i]));
	}
	trainList.append($liList)
	for(i=index-1;i<trainListArr.length;i++)
	{
		$(trainListArr[i]).find(".indexOfTrain")[0].innerText=i+1;
		trainList.append($(trainListArr[i]).data("trainObj",trainObjArr[i]));
	}
	
	//更新序号
	/*trainListArr=trainList.children("li");
	trainList.empty();
	for(i=0;i<trainListArr.length;i++)
	{
		$(trainListArr[i]).find(".indexOfTrain")[0].innerText=i+1;
		trainList.append(trainListArr[i]);
	}
	*/
	//trainList.append($liList);
	//更新交路名
...
function clickChose () {
	$('#activeChange button').bind('click', function(){
...
function btnLoadSelectDate () {
	...
	for(var i=0; i<days+1; i++) {
		var date = null;
		if(0==i) {
			date = m.add("day", 0).format('YYYY年MM月DD日');
		}
		else {
			date = m.add("day", 1).format('YYYY年MM月DD日');
		}		
		selectDate.push(date);
	}
	$("#liOptionms2side__sx").empty();
	$("#liOptionms2side__dx").empty();
	for(var i=0;i<selectDate.length;i++){
    	$("#liOptionms2side__sx").append("<option value='"+selectDate[i]+"'>"+selectDate[i]+"</option>");
	}
}
...
/**
*点击“日期”弹框中“已选区”旁“确定”按钮显示在下方textare中
*/
function sureSave () {
    var timeStr = $("#liOptionms2side__dx option").map(function(){return $(this).val();}).get().join(", ");
    $("#include_days").text("");
    $("#include_days").text(timeStr);
}
...
function sureClose () {
	$("#J_input").val("");
    $("#J_input").val($("#include_days").val());
    $('#regularSet').hide();
	$('#regularSet').dialog('close');
	$("#liOptionms2side__sx").empty();
	$("#liOptionms2side__dx").empty();
	$("#include_days").text("");
}
