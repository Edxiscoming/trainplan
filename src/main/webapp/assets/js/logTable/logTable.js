	            	   ...
	            	   formatter:function(value,row,index){
	            		   if(currentUserBureau===row.tokenVehBureau){
	            			   if(row.exceptionflag&&row.exceptionflag!==0){
	            				   return '<a href="#" style="color:red;white-space: normal;" onclick="crossManage(\''+row.cmOriginalCrossId+'\')">'+value+'</a>';
	            			   }else{
	            				   return '<a href="#" style="white-space: normal;" onclick="crossManage(\''+row.cmOriginalCrossId+'\')">'+value+'</a>';
	            			   }
	            		   }else{
	            			   return value;
	            		   }
...
*/
function crossesSet () {
	if(!confirm("执行该操作会将方案中未设置开行规律的交路的开行规律统一设置为“每日”，将未设置开行状态的交路的开行状态统一设置为“开行”，是否继续执行？")){
		return;
	}
	 $.ajax({
		type: "POST",
		...
		data:JSON.stringify({loginBureau: currentUserBureau}),
		success:function(data){
			if(data.code===0){
