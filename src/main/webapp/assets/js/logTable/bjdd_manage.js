...
function checkData(){
		...
		for(var i = 0; i < crosses.length; i++){
			if(crosses[i].checkFlag == 1 || crosses[i].checkFlag == -1){  
				...
				relevantBureau += (relevantBureau == "" ? "" : ",");
				relevantBureau += crosses[i].relevantBureau;
				relevantBureaus.push(relevantBureau);
			} 
...
function timeOpen(baseTrainId,trainNbr) {
  ...
  var columns =[[
    ...
    {field:'stnName_cz',title:'车站',rowspan:2,width:9,align:"center",
   	  formatter:function(value,row,index){
   		if(row.nodeStationName!=null){
			 return row.nodeStationName;
	    }else{
	    	 return row.stnName;
	    }
    }},
  ...
  crossManageData.$xiangTable.datagrid({onLoadSuccess:function(data){
	  	...
	  	for(var i=0;i<xiangItem.length;i++){
	  		if(xiangItem[i].stationFlag!="BTZ"){
	  			...
	  			if(xiangItem[i].nodeStationName!=null){
	  				jianItem.stnName1= xiangItem[i].nodeStationName;
	  			}else{
