            	...
            	{    var train = data.trainList[i];
                     ...
                     if(train.commonlineRule===null){
                    	 if(train.appointWeek!==null){
                        	 appointType = "weekTab";
                        	 $("input[target='#weekTab']").prop("checked",true); 
                        	 ...
                        	 if(train.appointDay!==null){
                            	 ...
                            	 appointType = "dateTab";
                            	 $("input[target='#dateTab']").prop("checked",true); 
                            	 $("#dateTab").addClass("active");
                            	 //先移除所有的复选框的选中
                            	 for(var i=1;i<=31;i++)
                        		 {
                        			 $("input[name='apointDaySolid'][value='"+i+"']").prop("checked",false); 
                        		 }
                            	 if(appointDay.indexOf(",")>0){
                            		 var days = appointDay.split(",");
                            		 /*$("#apointDaySolid").val(days);*/
                            		 $("input[name='apointDaySolid']").prop("checked",false);
                            		 for(i=0;i<days.length;i++)
                            		 {
                            			 var key=days[i];
                            			 
                            			 $("input[name='apointDaySolid'][value='"+key+"']").prop("checked",true); 
                            		 }
                            	 }else{
                            	 ...
                            	 if(train.appointPeriod!==null){
                            		 $("input[target='#cycleTab']").prop("checked",true); 
                            		 ...
                            		 $("input[name='commonLineRule'][value='1']").prop("checked",true);
                    	 ...
                    	 $("#commonTab").addClass("active");
                    	 $("input[name='commonLineRule'][value='"+appointValue+"']").prop("checked",true);
                     }
...
//	
	...
	switch(runType){
		...
		case "dateTab" : 
			...
			if(operationValue===""){
				var tempDates = $("input[name='apointDaySolid']:checked");
				for(var i=0;i<tempDates.length;i++){
					operationValue += ","+tempDates[i].defaultValue;
				}
	...
	$.ajax({
		...
		success: function (json) {
			...
			$.each(trainList, function (index, val) {
				...
				var $liList = $('<li class="row" isexist="1"><label class="control-label pull-left"><input style="margin-right:5px;" type="radio" name="crossIndex"><span class="indexOfTrain">' + (index+1)+'</span><span class="trainNo">．'+trainName+ '</span><span style="margin-left:20px;">'+val.sourceNode+'—'+val.targetNode+'</span></label>'+
...
function initHeaderBureau(){
	$.ajax({
		...
		success:function(data){
			...
			var $headerBureau=$("#headerBureau");
			//$headerBureau.val(currentUserBureau);
...
function searchCross(){
	...
	var param = {
		...
		tokenVehBureau : headerBureau,
//		highlineFlag :trainType,
	...
	crossGloble.$table.datagrid("options").url = url;
...
function initTable(){
	var columns =[[
	            	   ...
	            	   formatter:function(value,row,index){
	            		   if(currentUserBureau===row.tokenVehBureau){
	            			   if(row.exceptionflag&&row.exceptionflag!==0){
	            				   return '<a href="#" style="color:red;white-space: normal;" >'+value+'</a>';
	            			   }else{
	            				   return '<a href="#" style="white-space: normal;" >'+value+'</a>';
	            			   }
	            		   }else{
	            			   return value;
	            		   }
	               ...
	               {field:'crossSection',title:'运行区段',width:80,align:"center"},
	               {field:'sourceNode',title:'发站',width:50,align:"center"},
	               {field:'sourceTime',title:'始发时间',width:40,align:"right",
	            	   formatter:function(value,row,index){
	               ...
	               {field:'targetNode',title:'到站',width:50,align:"center"},
	               {field:'targetTime',title:'终到时间',width:45,align:"right",
	            	   formatter:function(value,row,index){
	               ...
	               {field:'alternateTrainNbr',title:'交替车次',width:60,align:"center",
	            	   formatter:function(value,row,index) {
		       				var html = value;
		       		        if(value==0 || !value){
		       		            html= "<input type=text name='alternateTrainNbr' style='width:140px'>";
		       		        }
		       		        else html="<input type=text name='alternateTrainNbr' style='width:140px;text-align:center' value="+value+">";
		       		        return html;
		       		    }
	               },
	               {field:'alternateDate',title:'交替日期',width:60,align:"center",
	            	   formatter:function(value,row,index) {
		       				var html = value;
		       		        if(value==0 || !value){
		       		            html= "<input type=text name='alternateDate' style='width:140px'>";
		       		        }
		       		        else if(row.autoFlag==0)
		       		        {
		       		        	html="<input type=text name='alternateDate' style='width:140px;text-align:center' value="+value+">";
		       		        }
		       		        else{
		       		        	html="<input type=text name='alternateDate' style='width:140px;text-align:center;background-color:#e3dd55' value="+value+">"
		       		        }
		       		        return html;
		       		    }},
	               ]];
...
function initTable2(){
	var columns =[[
	               ...
	               {field:'cmOriginalCrossId2',hidden:true},
	               {field:'cmOriginalTrainId2',hidden:true},
	               {field:'exceptionflag2',hidden:true},
	               {field:'squeueNo2',title:'序号',width:20,align:"center"},
	               /*{field:"ck2",checkbox:true,width:30,align:"center"},*/
	               {field:'crossName2',title:'交路',width:200,align:"left",
	            	   formatter:function(value,row,index){
	            		   if(currentUserBureau===row.tokenVehBureau){
	            			   if(row.exceptionflag&&row.exceptionflag!==0){
	            				   return '<a href="#" style="color:red;white-space: normal;" >'+value+'</a>';
	            	   ...
	            	   {field:'trainName2',title:'车次',width:70,align:"center",
	            		   formatter:function(value,row,index){
	            			   if(currentUserBureau===row.tokenVehBureau){
	            				   if(row.exceptionflag&&row.exceptionflag!==0){
	            		   ...
	            		   {field:'crossSection2',title:'运行区段',width:80,align:"center"},
	            		   {field:'sourceNode2',title:'发站',width:50,align:"center"},
	            		   {field:'sourceTime2',title:'始发时间',width:40,align:"right",
	            			   formatter:function(value,row,index){
	            				   //value=2016-01-10 18:55:00 ||2016-01-10 8:55:00
	            				   if(!value)
	            					   return ;
	            		   ...
	            		   {field:'targetNode2',title:'到站',width:50,align:"center"},
	            		   {field:'targetTime2',title:'终到时间',width:45,align:"right",
	            			   formatter:function(value,row,index){
	            				   if((!value)||(!row.sourceTime2))
	            					   return;
	            				   var html="";
	            				   //计算日期间隔
	            				   var from=row.sourceTime2.substring(0,10).split("-");
	            				   var to=value.substring(0,10).split("-");
	            				   from=new Date(from[0],from[1],from[2]);
	            				   to=new Date(to[0],to[1],to[2]);
	            				   var day=parseInt(Math.abs(from-to))/1000/3600/24;
	            		   ...
	            		   {field:'alternateTrainNbr2',title:'交替车次',width:60,align:"center"},
	            		   {field:'alternateDate2',title:'交替日期',width:60,align:"center"},
	            		   {field:'useStatus',title:'操作',width:60,align:"center",
	    	                   formatter:function(value,row,index) {
	    	                		var html = '<div style="text-align:center;">'+
	                        			  '<a href="#" style="margin-left:10px;" onclick="trainManage(\''+row.cmOriginalCrossId2+'\')">修改</a>'+
	                        			  '<a href="#" style="margin-left:10px;" onclick="deleteTrain(\''+row.cmOriginalCrossId2+'\')">删除</a></div>';
...
//		   			highlineFlag:-1 //全部
	...
	//var url = basePath + "/cmOriginalCross/pageQueryCrossAndTrain";
	var targetTable = $("#crossTable2");
	crossGloble.$table2 = targetTable.datagrid({
		height: 'auto',
		width:'100%',
		nowrap: true,
		striped: true,
		...
		onClickRow:function(rowIndex, rowData){
			crossGloble.$table2.datagrid("clearSelections");
			crossGloble.$table2.datagrid("clearChecked");
			var cmOriginalCrossId2 = rowData.cmOriginalCrossId2;
			var rows = crossGloble.$table2.datagrid("getRows");
			var firstMatch = true;
			for(var i=0;i<rows.length;i++){
				if(rows[i].cmOriginalCrossId2===cmOriginalCrossId2){
					crossGloble.$table2.datagrid("selectRow",i);
					if(firstMatch){
						crossGloble.$table2.datagrid("checkRow",i);
						firstMatch = false;
					}
				}
			}
 ...
 */
function trainManage(cmOriginalCrossId){
	crossGloble.$table2.datagrid("clearSelections");
	var allRow=crossGloble.$table2.datagrid("getRows");
	for(var i =0;i<allRow.length;i++)
	{
		if(allRow[i].cmOriginalCrossId2==cmOriginalCrossId)
			crossGloble.$table2.datagrid("selectRow",i);
	}
	
	var row=crossGloble.$table2.datagrid("getSelections");
	crossGloble.$table2.datagrid("clearSelections");
	var objs=new Array();
	for(i =0;i<row.length;i++)
	{
		objs[i]=new Object();
		for(var key2 in row[i])
		{
			var key=key2.substring(0,key2.length-1);
			objs[i][key]=row[i][key2];
		}
		objs[i].squeueNo=i+1;
	}
	//先清空表格
	crossGloble.$table.datagrid('loadData', { total: i, rows: objs});
	var $p = crossGloble.$table.datagrid('getPager');
    $p.pagination({
        showPageList: false,
        showRefresh: false,
        beforePageText: '第',//页数文本框前显示的汉字
        afterPageText: '页    共 {1} 页',
        displayMsg: '当前显示 {1} - {1}条记录       共 {1} 条记录'
    });
 ...
 * @param crossId 交路id
 */
function deleteTrain(cmOriginalCrossId){
	$.extend($.messager.defaults,{  
	    ok:"确定",  
	    cancel:"取消"  
	});  
	$.messager.confirm('删除交路','是否确定删除此交路？',function(yes){
		if(yes){
			//var row=crossGloble.$table2.datagrid("getSelections");
			/*var rows=crossGloble.$table2.datagrid("getRows");
			for(var i = 0;i<rows.length;i++)
			{
				if(rows[i].cmOriginalCrossId2==cmOriginalCrossId)
					crossGloble.$table2.datagrid("selectRow",i);
			}*/
			
			var rows=crossGloble.$table2.datagrid("getSelections");
			var rowIndex;
			for(var i=0;i<rows.length;i++)
			{
				rowIndex=crossGloble.$table2.datagrid("getRowIndex",rows[i]);
				crossGloble.$table2.datagrid("deleteRow",rowIndex);
			}
			
			
			//主要是为了合并序号
			mergeRow(crossGloble.$table2.datagrid("getRows"));
 ...
 * 开行规律切换事件
 */
function bindRunRule(){
	$(".runRule").on("click",function(e){
		var targ;
		if (!e){
			e = window.event;
		}
		...
		var toShow = $(targ).attr("target"); 
		$(".tab-pane").removeClass("active");
		$(toShow).addClass("active");
		//$(".setDate").datepickerRefactor({target:"#apointDayDateSolid"});
	});
}
/**
*批量设置
...
function crossesSet() {
    ...
    $('#setDateTime').dialog('open');
}
function confirmBatchConfig(){
	var ToSetTime=$("#ToSetTime").val();
	ToSetTime=ToSetTime.replace(/-/g,"");
	 $.ajax({
		type: "POST",
       dataType: "json",
       contentType : "application/json",
		url: basePath + "/cmOriginalCross/batchConfigAlternateInfo",
		data:JSON.stringify({date: ToSetTime,loginBureau:currentUserBureau}),
		success:function(data){
			if(data.code===0){
	 ...
	 $('#setDateTime').dialog('close')
}
//在下面新增一行
function addRow(){
	row=crossGloble.$table.datagrid("getSelections");
	objs=new Array();
	for(var i =0;i<row.length;i++)
	{
		...
		{
			var key2=key+"2";
			objs[i][key2]=row[i][key];
		}
		
		var rowIndex=crossGloble.$table.datagrid("getRowIndex",row[i]);
		objs[i].alternateTrainNbr2=$("input[name='alternateTrainNbr']")[rowIndex].value,
		objs[i].alternateDate2=$("input[name='alternateDate']")[rowIndex].value
		...
		var ids=new Array();
		if($('#crossTable2').datagrid("getRows").length>0)
		{
			for(var j=0;j<$('#crossTable2').datagrid("getRows").length;j++)
			{
				ids[j]=$('#crossTable2').datagrid("getRows")[j].cmOriginalTrainId2;
			}
		
			for(var j=0;j<ids.length;j++)
			{
				if(objs[i].cmOriginalTrainId2==ids[j])
				{//如果相等应该更新
					$('#crossTable2').datagrid('updateRow',{
						index: j,
						row: {
							alternateTrainNbr2: objs[i].alternateTrainNbr2,
							alternateDate2: objs[i].alternateDate2
						}
					});
			...
			if(flag)
			{
				$('#crossTable2').datagrid('appendRow',objs[i]);
			}
		}
		else{
			$('#crossTable2').datagrid('appendRow',objs[i]);
	...
	//merge
	mergeRow($('#crossTable2').datagrid("getRows"))

}

//批量保存
function addBatchRow(){
	rows=$('#crossTable2').datagrid("getRows");
	if(rows.length==0)
	{
		return;
	}
	...
	var crossArr = [];
	var trainArr = [];
	var cmOriginalCrossId = "";
	for(var i=0;i<rows.length;i++)
	{
		if(cmOriginalCrossId == "" || cmOriginalCrossId != rows[i].cmOriginalCrossId2){
			cmOriginalCrossId = rows[i].cmOriginalCrossId2;
			var cross = {};
			cross.cmOriginalCrossId = rows[i].cmOriginalCrossId2;
			crossArr.push(cross);
		}
		var train = {};
		train.cmOriginalTrainId=rows[i].cmOriginalTrainId2;
		train.cmOriginalCrossId=rows[i].cmOriginalCrossId2;
		train.alternateDate=rows[i].alternateDate2;
		train.alternateTrainNbr=rows[i].alternateTrainNbr2;
		trainArr.push(train);
	}
	var param = {crosses : crossArr, trains : trainArr};
	$.ajax({
		type: "POST",
        dataType: "json",
        contentType : "application/json",
...
function mergeRow(rows){
	...
	var currentCrossId= list[0].cmOriginalCrossId2;
	var count = 0;
	var index = 0;
	var merges = [];
	var merge=new Object();
	merge.index=0;
	for(var i=0;i<list.length;i++){
		if(currentCrossId==list[i].cmOriginalCrossId2){
			merge.rowspan=++count;
			if(i===list.length-1){
				merges.push(merge);
			}
			...
			currentCrossId = list[i].cmOriginalCrossId2;
			count=1;
		}
		
	}
		for(var i=0; i<merges.length; i++){
			crossGloble.$table2.datagrid('mergeCells',{
				index:merges[i].index,
				field:'crossName2',
				rowspan:merges[i].rowspan
			});
