...
function initTable(){
	var columns =[[
	       	    {field:"ck",checkbox:true,width:5,rowspan:2,align:"center"},
	       		...
	       		{field:'crossName',title:'车底交路',width:25,align:"left",rowspan:2,sortable:true},
       		    ...
       		    {field:'spareFlag',title:'开行状态',width:10,align:"center",rowspan:2,sortable:true,
       		    	formatter:function(value,row,index){
       		    		if(value!=null)
       		    		{
       		    			value=value.replace(/1/g,"开行").replace(/2/g,"备用").replace(/0/g,"停用")
       		    			var valueArr=value.split("-");
       		    			var flag=true;   //默认全部相等
	       		...
	       		{field:'tokenVehBureau',title:'担当局',width:5,align:"center",rowspan:2,sortable:true,
	       			formatter:function(value,row,index) {
                   ...
                   {field:'relevantBureau',title:'相关局',width:20,align:"center",sortable:true,
   	       		...
   	       		{field:'checkFlag',title:'本局',width:5,align:"center",sortable:true,
	       			formatter:function(value,row,index) {
	       		        var html = "";
	       		        if(row.check_wor == 1){
	       		            html= '<div style="background:#48a048;border-radius:10px; width:30px; text-align: center;margin: 0px auto;color: #FFFFFF;">已</div>';
	       		        }else if (row.check_wor == -1) {
	       		            html = '<div style="background:#ef6b00;border-radius:10px; width:30px;text-align: center;margin: 0px auto;color: #FFFFFF;">不</div>';
	       		        }else {
...
//					highlineFlag:"-1"
	       	...
	       	crossManageData.$table.datagrid("options").singleSelect = false;
}
 ...
 * 查询
 */
function searchData(tokenFlag,obj){
	//方案
	var cmVersionId = $("#cmVersionId").val();
...
function changeData(){
	 var allrow=$('#mcrossTable').datagrid("getRows");
	 console.log(allrow);
	 
	 var dateArray=new Array();
	 for(var i=0;i<allrow.length;i++)
	 {
		 if(!allrow[i].createCrossFlag&&!allrow[i].checkFlag )
		 {
			 var obj=new Object();
			 obj.cross={
				 cmOriginalCrossId:allrow[i].cmOriginalCrossId,
				 pairNbr:$("input[name='pairNbr']")[i].value,
				 groupTotalNbr:$("input[name='groupTotalNbr']")[i].value
			 }
...
function checkDataNo(){
		...
		if(crossIds == ""){
		 ...
		 $.ajax({
				url : basePath+"/cmOriginalCross/checkCrossNotPass",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
...
function generateCross(){
	...
	for(var i =0;i<cross.length;i++)
	{
		...
		if (cross[i].checkFlag  != 1) {
			alertMessage("warning","必须要审核通过才能生成交路！");
			return;
		}
		if(cross[i].groupTotalNbr < 1) {
...
function clickUnitRow (row) {
	...
	var trainNbr=row.trainNbr;
	timeOpen(timeId,trainNbr);
    $("#crossHideId").val(timeId);
    $('#timeTable').show().dialog({
	      title: '时刻表:车次：' + trainNbr +"&nbsp;&nbsp;"+ row.startStn + '——' + row.endStn,
	      width:1000,
	      height:800,
	      cache: false,
	      modal: true
...
function timeOpen(baseTrainId,trainNbr) {
  ...
  var columns =[[
    ...
    {field:'stnName_cz',title:'车站',rowspan:2,width:9,align:"center",
     	  formatter:function(value,row,index){
         	return row.stnName;
      }},
    //{field:'nodeStationName',title:'车站',rowspan:2,width:5,align:"center"},
    {field:'nodeName',title:'节点',rowspan:2,width:9,align:"center"},
    {field:'arrTime',title:'到达时间',rowspan:2,width:9,align:"center",sortable:true},
    {field:'dptTime',title:'出发时间',rowspan:2,width:9,align:"center",sortable:true},
    ...
    {field:'trackName',title:'股道',rowspan:2,width:5,align:"center",sortable:true},
    {field:'stnName_zt',title:'站台',rowspan:2,width:5,align:"center",sortable:true,
   	  formatter:function(value,row,index){
   	       	return row.stnName;
   	  }},
    {field:'planForm',title:'作业',rowspan:2,width:6,align:"center",sortable:true},
    {field:'jobs',title:'办客',rowspan:2,width:10,align:"center",sortable:true,
    	formatter:function(value,row,index){
     	return row.jobs.indexOf("客运营业")>0?"是":"--";
    }},
    ...
    {field:'nodeTdcsId',title:'交替时间',rowspan:2,width:8,align:"center",sortable:true,
      ...
      {field:'arrTrainNbr',title:'到达',width:5,align:"center"},
      {field:'dptTrainNbr',title:'出发',width:5,align:"center"},
  ...
  var jianColumns = [[
                      ...
                      {field:'stnName1',title:'车站',rowspan:2,width:9,align:"center"},
                      {field:'nodeName1',title:'节点',rowspan:2,width:9,align:"center"},
                      {field:'arrTime1',title:'到达时间',rowspan:2,width:9,align:"center",sortable:true},
                      {field:'dptTime1',title:'出发时间',rowspan:2,width:9,align:"center",sortable:true},
                      {title:'车次',colspan:2,align:"center"},
                      {title:'天数',colspan:2,align:"center"},
...
//                        }
                      ...
                      {field:'trackName1',title:'股道',rowspan:2,width:5,align:"center",sortable:true},
                      {field:'nodeTdcsName1',title:'站台',rowspan:2,width:5,align:"center",sortable:true},
                      {field:'planForm1',title:'作业',rowspan:2,width:6,align:"center",sortable:true},
                      {field:'jobs1',title:'办客',rowspan:2,width:10,align:"center",sortable:true},
                     /* {field:'kyyy1',title:'普速规律',rowspan:2,width:10,align:"center",sortable:true},
                      {field:'nodeTdcsId1',title:'交替时间',rowspan:2,width:8,align:"center",sortable:true},*/
                        ...
                        {field:'arrTrainNbr1',title:'到达',width:5,align:"center"},
                        {field:'dptTrainNbr1',title:'出发',width:5,align:"center"},
  ...
  crossManageData.$xiangTable.datagrid({onLoadSuccess:function(data){
	  	...
	  	for(var i=0;i<xiangItem.length;i++){
	  		if(xiangItem[i].stationFlag!="BTZ"){
	  			...
	  			jianItem.stnName1= xiangItem[i].stnName;
	  			...
	  			jianItem.jobs1= xiangItem[i].jobs.indexOf('客运营业')>0?'是':'--';
	  			// jianItem.kyyy1= xiangItem[i].kyyy;
	  			// jianItem.nodeTdcsId1= '';//先不显示xiangItem[i].nodeTdcsId;
	  			jianItem.arrTrainNbr1= xiangItem[i].arrTrainNbr;
	  			jianItem.nodeTdcsName1= xiangItem[i].stnName;
