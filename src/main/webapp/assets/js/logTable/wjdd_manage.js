...
function initTable(){
	var columns =[[
	       	    ...
	       	    {field:'cmCrossId',hidden:true},
	       	    {field:'appointWeek',hidden:true},
	       	    {field:'appointDay',hidden:true},
	       	    {field:'appointPeriod',hidden:true},
	       		{field:'crossName',title:'车底交路',width:25,align:"left",sortable:true},
	       		{field:'commonlineRule',title:'开行规律',width:15,align:"left ",sortable:true,formatter:function(value,row,index) {
	       		...
	       		{field:'checkFlag',title:'审核状态',width:5,align:"center",sortable:true,
	       			formatter:function(value,row,index) {
	       		        var html = "";
	       		        if(value == 1){
...
function openGraph(cmOriginalCrossId){
	...
	for(var i =0;i<cross.length;i++)
	{
		...
		if (typeof(cross[i].commonlineRule) == "undefined" || cross[i].commonlineRule == "" || cross[i].commonlineRule == null) {
			alertMessage("warning","开行规律未填，不能预览交路！");
			return;
		} 
		if (getRules(cross[i]).indexOf("--") > -1) {
			alertMessage("warning","开行规律填写不正确，不能预览交路！");
			return;
		};
	}
