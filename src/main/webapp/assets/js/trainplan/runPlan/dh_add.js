...
//					} 
	...
	self.saveDhPlanCross = function() {
		if('' == $("#jlName").val() || null == $("#jlName").val()){
			showWarningDialog("请填写交路名！");
			return;
		}
//		if('' == $("#userName").val() || null == $("#userName").val()){
//			showWarningDialog("请填写联系人！");
//			return;
//		}
		if('' == $("#userTel").val() || null == $("#userTel").val()){
			showWarningDialog("请填写联系方式！");
			return;
		}
		if('' == $("#userReason").val() || null == $("#userReason").val()){
			showWarningDialog("请填写打回原因！");
			return;
		}
//		console.log($("#type").val());
		commonJsScreenLock();
		$.ajax({
			url : basePath+"/runPlan/saveDhPlanCrosses",
			cache : false,
			type : "POST",
			dataType : "json",
			contentType : "application/json",
			data :JSON.stringify({
				crossName : $("#jlName").val(),
				//userName:$("#userName").val(),
				userTel: $("#userTel").val(),
				userReason: $("#userReason").val(),
				startTime:$("#startTime").val(),
				endTime: $("#endTime").val(),
				planCrossIds: $("#planCrossIds").val(),
				type: $("#type").val()
			}),
			success : function(result) {
				...
				if (result != null && typeof result == "object" && result.code == "0") {
					...
					window.parent.cross.loadCrosses();
					window.parent.cross.loadCheckplans($("#planCrossIds").val());
