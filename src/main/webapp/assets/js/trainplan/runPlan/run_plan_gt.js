		...
		$
				.ajax({
					...
					data : JSON.stringify({
						...
						highlineFlag : "1",// 高铁//highlingFlag == null ? null :
											// highlingFlag.value,
...
//				sendCross.push(crosses[i]);
		...
		$("#run_plan_sendMsg").find("iframe").attr("src", basePath+"/runPlan/runPlanSendMsg?crossIds="+crossIds+"&crossName="+crossName+"&type=1");
...
//					if(crosses[i].checkedBureau().indexOf(currentUserBureau) == -1){
						...
						delCrosses.push(crosses[i]);
//					}else{
//						console.log("cbB = true");
//						cbB = true;
//					}
//				}else{
//					console.log("cfB = true");
//					cfB = true;
//				}
			}
		}
...
//				return;
//			}else if(cbB){
//				showWarningDialog("已审局不能包含'" + currentBureauShortName + "'！");
//				return;
//			}else 
				if (crossIds == "") {
...
//			console.log("2094H::" + row.planCrossId());
...
function CrossRow(data) {
	...
	self.crossSection = ko.observable(data.crossSection == 'null' ? ""
			: data.crossSection);
