...
function heightAuto(){
	...
	//alert("window: 860---"+WH);
	$("#plan_view_div_palnDayDetail").css("height",WH-570+"px");
};
...
//				+ self.createRunPlanTotalCount() + "个交路，目前已成功生成："
//				+ self.createRunPlanCompletedCount() + "个交路的开行计划，另有："
//				+ self.createRunPlanErrorCount() + "个交路生成开行计划失败";
	});

	// 更新列车某天的开行情况
	self.updateTrainRunPlanDayFlag = function(runPlan) {
		$.each(self.trainPlans(), function(x, n) {
			if (n.unitCrossId == runPlan.unitCrossId
					&& n.trainNbr == runPlan.trainNbr 
	...
	self.selectCross = function(row) {
		...
		if (row.selected() == 0) { // 整条交路全选
			...
			$.each(self.trainPlans(), function(i, train) {
				if (train.crossName == row.crossName) {
					train.selected(0);
					//$("#handCreatePlanCrossName").text(train.crossName);
					$.each(train.runPlans(), function(z, n) {
						if (n.createFlag() == 0) {
							n.selected(0);
							self.selectedRunPlan.remove(n);
	...
	self.init = function() {
		...
		//self.searchModle().cross_start_date(self.currdate());
...
//    	}
		...
		$.ajax({
								// url : basePath+"/cross/getUnitCrossInfo",
					url : basePath + "/runPlan/getTrainRunPlansForCreate",
					cache : false,
					type : "POST",
					dataType : "json",
					contentType : "application/json",
...
//																			console.log("trainSort=" + (i+1) + "," + b.trainSortBase);// trainSortBase
...
//												&& result.length >= 0) {
//											showSuccessDialog("正在生成开行计划");
//											// 2015-1-20 16:26:26,加载天数少于原有已生成天数,不能自动加载出来,所以直接再刷新一遍.
//											//loadCrosses();
//										} else {
//											showErrorDialog("生成开行计划失败");
//										}
										...
										success : function(result) {
											if(result.code == 0){
												if($("#totalNum")){
													...
													$("#successNum").html(result.data.successNum);
													$("#failNum").html(result.data.failNum);
													result.data.failCrossInfo;
													if(result.data.failNum == 0){
														$("#failNum").html(result.data.failNum);
													} else {
														$("#failNum").html("<a href='#' onclick='alert(\"以下交路生成失败，请检查数据：" + result.data.failCrossInfo + "\")'>"+result.data.failNum+"</a>");
													}
												}
											}
											$("#generateInfo").show();
...
function TrainRunPlanRow(data) {
	...
	self.createStatusShowValue = ko
			.computed(function() {
				...
				case 2:
					return self.crossName + "（" + self.cross_start_date + "）"
							+ "&nbsp;&nbsp;<span class='label label-success'>(已生成开行计划)</span>";// "(已生成开行计划)";
					break;
				case -1:
					return self.crossName + "（" + self.cross_start_date + "）"
							+ "&nbsp;&nbsp;<span class='label label-danger'>(发生异常)</span>";// "(发生异常)";
					break;
				case -2:
					return self.crossName + "（" + self.cross_start_date + "）"
	 ...
	 * 打开交路图
	 */
	self.showWayGraph = function(){
		var startDate = $("#runplan_input_startDate").val(); 
		var endDate =  $("#runplan_input_endDate").val();   
		getGraphData(self.planCrossId,startDate,endDate);
	};
	// 初始化开行计划的列表
