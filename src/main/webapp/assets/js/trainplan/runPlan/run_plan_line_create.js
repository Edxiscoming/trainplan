...
function updateTrainRunPlanStatus(message){  
	console.log("updateTrainRunPlanStatus：：" + message);
...
function CrossModel() {
	...
	self.completedMessage = ko.computed(function(){ 
		...
		//return self.createRunPlanTotalCount() == 0 ? "" : "选中：" + self.createRunPlanTotalCount() + "个计划，目前已成功生成：" + self.createRunPlanTotalCount() + "个运行线，另有：" + self.createRunPlanErrorCount() + "个运行线失败";
	});
...
//		    }); 
	...
	self.text = function(tokenVehBureau){
		switch (tokenVehBureau) {
			case 'P':return "京";break;
			case 'B':return "哈";break;
			case 'T':return "沈";break;
			case 'V':return "太";break;
			case 'C':return "呼";break;
			case 'F':return "郑";break;
			case 'N':return "武";break;
			case 'Y':return "西";break;
			case 'K':return "济";break;
			case 'H':return "上";break;
			case 'G':return "南";break;
			case 'Q':return "广";break;
			case 'Z':return "宁";break;
			case 'W':return "成";break;
			case 'M':return "昆";break;
			case 'J':return "兰";break;
			case 'R':return "乌";break;
			case 'O':return "青";break;
		}
	}
...
//		$("#searchModleHiddenValue").val(chart.name);
		...
		$.ajax({
				...
				/*
				 * 	self.canCheck = ko.computed(function(){
				//		var code1 = planCross.tokenVehBureauShowValue();
				//		var currentBureau = $("#currentBureauShortName").val();
				//		if(currentBureau==code1){
				//			return true;
				//		}
				//		else{
				//			return false;
                   ...
                   var rows = [];
					if (result != null && result != "undefind" && result.code == "0") { 
						if(self.searchModle().searchType() != '3'){//图定
							...
							var cn;
							var pcId = "";
//							var nbr = "05133-D638/5-D636/7-D638/5-D636/7-D5134";
									...
									if(n.runDay != null){
										...
										$.each(result.data.data, function(rdd, rdd1){
											if(rdd1.groupSerialNbr == 1 || rdd1.groupSerialNbr == null){
												if(n.crossName == rdd1.crossName && n.planCrossId == rdd1.planCrossId){
													if(tn != rdd1.trainNbr){
...
//								 console.log("↑↑↑↑" +  n.trainNbr + "," + sort + "↓↓↓↓" +  t.trainNbr + "," + t.trainSort);
							 ...
							 $.each(result.data.data, function(z, n){ 
								 ...
								 $.each(self.trainPlans(), function(x, t){ 
										if(t.trainNbr == n.trainNbr&&t.telCmdId==n.telCmdId){
											for(var i = 0; i < t.runPlans().length; i++){ 
												if(t.runPlans()[i].day.replace(/-/g, "") == n.runDay){ 
													t.runPlans()[i].runFlag(parseInt(n.runFlag));
													t.runPlans()[i].createFlag(parseInt(n.createFlag));
...
function BureausRow(data) {
	var self = this;  
//	self.canCheck = ko.observable(false); 
