					...
					success : function(result) {   
						if (result != null && result != "undefind" && result.code == "0") { 
							for ( var i = 0; i < result.data.length; i++) {
								if(result.data[i].crewGroup!=null&&result.data[i].crewGroup!=""&&(result.data[i].crewType=="1"||result.data[i].crewType=="2"||result.data[i].crewType=="3")){
...
//										row.isMatch("0");
		...
		_self.exportExcel = function() {
			...
			window.open(basePath+"/crew/highline/exportExcel/1/"+_runDate+"/"+_trainNbr+"/"+_highlineFlag);
		};
		...
		_self.uploadExcel = function() {
	        ...
	        $.ajaxFileUpload ({
                ...
                success: function (data) {
                	if(data.code=='101'){
                		showErrorDialog("请按模板导入");
                		  $("#loading").hide();
                		$("#uploadCrewExcelModal").modal('hide'); 
                		$("#btn_fileToUpload").removeAttr("disabled");
                		return;
                	}else if(data.code=='401'){
                		showSuccessDialog("导入失败");
                		 $("#loading").hide();
                		$("#uploadCrewExcelModal").modal('hide'); 
                		$("#btn_fileToUpload").removeAttr("disabled");
                		return;
