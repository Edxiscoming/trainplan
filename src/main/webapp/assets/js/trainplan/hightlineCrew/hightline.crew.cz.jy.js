...
var gloabBureaus = [];
	...
	function SearchModle(){
		...
		_self.crewGroupOption =  ko.observable();
		_self.crewGroups =  ko.observableArray();
		
		_self.crewGroups1 =  ko.observableArray();
		_self.initData = function() {
			_self.runDate(currdate());
			//_self.crewGroups.push({"crewGroupName": "", "crewHighlineId": ""});
					...
					success : function(result) {   
						if (result != null && result != "undefind" && result.code == "0") { 
							for ( var i = 0; i < result.data.length; i++) {
								if(result.data[i].crewGroup!=null&&result.data[i].crewGroup!=""&&(result.data[i].crewType=="11"||result.data[i].crewType=="12")){
									if(_self.crewGroups().length == 0){
										_self.crewGroups.push({"crewGroupName": result.data[i].crewGroup, "crewHighlineId": result.data[i].crewHighlineId});
									}else{
										var hasBoolean = true;
										for( var j = 0; j < _self.crewGroups().length;j++){
											if(_self.crewGroups()[j].crewGroupName==result.data[i].crewGroup){
												hasBoolean = false;
											}
										}
										if(hasBoolean == true){
							...
							//console.dir(_self.crewGroups().length)
						} else {
	...
	function PageModel() {
		...
		_self.currentTrainInfoMessage = ko.observable();//时刻表头信息
...
//		};
		...
		_self.onAddOpen = function() {
				...
				var title = "";
				var cmdTypeText = "";
				path = "/runPlanLk/czAddPageJy";
				title = "新增填报车长";
				cmdTypeText = "新增填报车长";
				$("#add_cz_info_dialog").find("iframe").attr("src", basePath + path).attr("id", cmdTypeText);
				$('#add_cz_info_dialog').dialog({title: title, autoOpen: true, modal: false, draggable: true, resizable:true,
					onResize:function() {
						var iframeBox = $("#add_cz_info_dialog").find("iframe");
						var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;
						var WH = $('#add_cz_info_dialog').height();
						var WW = $('#add_cz_info_dialog').width();
		                if (isChrome) {
...
//			_self.wdCmdTrain(new WdCmdTrainModel(createEmptyCmdTrainRow()));//创建一个空对象
		...
		_self.onEditOpen = function() {
			...
			var title = "";
			var cmdTypeText = "";
			path = "/runPlanLk/czEditPageJy";
			title = "新增填报司机";
			cmdTypeText = "新增填报司机";
			$("#add_cz_info_dialog").find("iframe").attr("src", basePath + path).attr("name", objSelect.crewHighlineId).attr("id", cmdTypeText);
			$('#add_cz_info_dialog').dialog({title: title, autoOpen: true, modal: false, draggable: true, resizable:true,
				onResize:function() {
					var iframeBox = $("#add_cz_info_dialog").find("iframe");
					var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;
...
//										row.isMatch("0");
		...
		function loadHightLineCrewSjDataForPage(startIndex, endIndex) {
			...
			var crewGroupName = null;
			if(_self.searchModle().crewGroupOption() && _self.searchModle().crewGroupOption()!=undefined){
				crewGroupName = _self.searchModle().crewGroupOption().crewGroupName;//乘务组编号
			}else{
				crewGroupName ="";
			}
		...
		_self.exportExcel = function() {
			...
			window.open(basePath+"/crew/highline/exportExcel/11/"+_runDate+"/"+_trainNbr+"/"+_highlineFlag);
		};
		...
		self.isShowRunPlans = ko.observable(true);// 显示开行情况复选框   默认勾选000
		...
		//#################################################################
	_self.showCrossTrainTimeDlg2 = function(){ 
		if(!_self.currentRowPlanTrainId() || _self.currentRowPlanTrainId() == ''){ 
			showWarningDialog("请先选择交路"); 
			return;
		} 
		var trainId = _self.currentRowPlanTrainId();  
		if($('#train_time_dlg').is(":hidden")){  
			$("#train_time_dlg").find("iframe").attr("src", basePath+"/jbtcx/queryPlanLineTrainTimesNewIframe?trainId=" + trainId);
			$('#train_time_dlg').dialog({ title: "时刻表:" +_self.currentTrainInfoMessage(), autoOpen: true, modal: false, draggable: true, resizable:true,
				onResize:function() {
...
//			     });
				$.ajax({
					url : basePath+"/jbtcx/queryPlanLineTrainTimesNewIframe?trainId="+row.planTrainId(),
					cache : false,
					type : "GET",
				});
//				_self.queryPlanLineTrainTimes(row);
		...
		_self.initPage = function () {
			...
			$("#add_cz_info_dialog").dialog("close");
			//页面元素绑定
			_self.searchModle().initData();
		    $.ajax({
...
function heightAuto(){
	...
	$("#left_height").css("height",WH-187+"px");
	$("#right_height").css("height",WH-122+"px");
};
