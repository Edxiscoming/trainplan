	...
	function SearchModle(){
		...
		_self.loadCrewGroup = function(crewGroups){   
			for ( var i = 0; i < crewGroups.length; i++) {
				...
				if(crewGroups[i].crewType=='1'||crewGroups[i].crewType=='2'||crewGroups[i].crewType=='3'){
...
//		};
		...
		_self.onAddOpen = function() {
				...
				var title = "";
				var cmdTypeText = "";
				path = "/runPlanLk/jxsAddPage";
				title = "新增填报机械师";
				cmdTypeText = "新增填报机械师";
				$("#add_jxs_info_dialog").find("iframe").attr("src", basePath + path).attr("id", cmdTypeText);
				$('#add_jxs_info_dialog').dialog({title: title, autoOpen: true, modal: false, draggable: true, resizable:true,
					onResize:function() {
						var iframeBox = $("#add_jxs_info_dialog").find("iframe");
						var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;
						var WH = $('#add_jxs_info_dialog').height();
						var WW = $('#add_jxs_info_dialog').width();
		                if (isChrome) {
...
//			_self.wdCmdTrain(new WdCmdTrainModel(createEmptyCmdTrainRow()));//创建一个空对象
		...
		_self.onEditOpen = function() {
			...
			var title = "";
			var cmdTypeText = "";
			path = "/runPlanLk/jxsEditPage";
			title = "新增填报司机";
			cmdTypeText = "新增填报司机";
			$("#add_jxs_info_dialog").find("iframe").attr("src", basePath + path).attr("name", objSelect.crewHighlineId).attr("id", cmdTypeText);
			$('#add_jxs_info_dialog').dialog({title: title, autoOpen: true, modal: false, draggable: true, resizable:true,
				onResize:function() {
					var iframeBox = $("#add_jxs_info_dialog").find("iframe");
					var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;
...
//										row.isMatch("0");
		...
		_self.exportExcel = function() {
			...
			window.open(basePath+"/crew/highline/exportExcel/3/"+_runDate+"/"+_trainNbr+"/"+_highlineFlag);
		};
...
//			     });
		...
		_self.initPage = function () {
			...
			$("#add_jxs_info_dialog").dialog("close");
			//页面元素绑定
			_self.searchModle().initData();
