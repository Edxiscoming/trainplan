...
function ApplicationModel() {
	...
	self.cmdTextStr = ko.observable("0");
...
//	self.trainCfIds = ko.observable();
	...
	self.showallcreat = function(){
		...
		commonJsScreenLock();
		$.ajax({
			url : "audit/plan/creatCmdInfo/" + date + "/" + type + "/"
			+ sentFlag + "/" + tokenVehBureauNbr + "/" + train_nbr
			+ "?trainType=0",
			cache : false,
			type : "GET",
			dataType : "json",
			...
			success : function(result) {
				self.cmdTextStr(result.data);
				$('#creatline').dialog("open");
			},
			error : function() {
				showErrorDialog("获取命令内容失败");
			},
			complete : function(){
				commonJsScreenUnLock();
			}
	...
	self.createCmdInfoCancel = function() {
		$("#creatline").dialog("close");//命令查看窗口div
	};
	/**
	 * 生成命令
	 */
	self.createCmd = function() {
		if(self.cmdTextStr()==null || self.cmdTextStr()=="") {
    	...
    	commonJsScreenLock();
		$.ajax({
			url : basePath+"/audit/createCmd",
			cache : false,
			type : "POST",
			dataType : "json",
			contentType : "application/json",
			data :JSON.stringify({
				cmdInfo: $("#textarea_cmdInfoStrs").val()//self.cmdInfoStr()
			}),
			success : function(result) {
...
//						  plan.isRoutingMatch(data.isRoutingMatch);
	...
	self.selectBox = function(row) {
		if (row.isSelected() == 0) { // 选中的时候
			self.allBtn(1);
			ko.utils.arrayForEach(self.tableModel().planList(), function(plan) {
				if (plan.isSelected() != 1 && plan != row
						&& plan.needLev1() == 1) {
					self.allBtn(0);
				}
			});
		} else {
			// ko.utils.arrayForEach(self.tableModel().planList(),
			// function(plan) {
			self.allBtn(0);
			// });
		}
																	...
																	if (null != resp.error) {
																		...
																		for ( var i = 0; i < self
																			...
																			for ( var j = 0; j < data.length; j++) {
																				console.log(data[j].planId);
																				console.log(self.tableModel().planList()[i].id());
																				if (data[j].planId == self.tableModel().planList()[i].id()) {
...
//						  plan.isRoutingMatch(data.isRoutingMatch);
	...
	self.showChatPia = function() {
		...
		if (!self.isShowCrossDetailInfo()) {// 勾选
			$("#chatId").show();
			// $("#body_right").attr("style","height: 530px; overflow-y: scroll;
			// padding: 15px; margin-bottom: 15px;margin:-5px -3px 10px -17px");
			$("#left_height").css({
				"height" : WH - 105 - 270 + "px"
			});
			$("#body_right").css({
				"margin-bottom" : "10px"
			});
			...
			// $("#body_right").attr("style","height: 795px; overflow-y: scroll;
			// padding: 15px; margin-bottom: 15px;margin: -5px -5px 0 -17px;");
		}
	}
...
function ParamModel() {
	...
	var date = moment(new Date((new Date() / 1000 + 86400 * 1) * 1000)).format(
			"YYYY-MM-DD");
	if (date) {
		...
		$.ajax(
				{
					url : "audit/plan/chart/traintype/" + date + "/"
							+ $("#train_type").val() + "?trainType=0&name="
							+ $("#train_nbr").val(),
					method : "GET",
					contentType : "application/json; charset=UTF-8"
				}).done(
...
function TableModel() {
	...
	self.loadTable = function() {
		...
		var url = "audit/plan/runplan1/" + date + "/" + type + "/" + sentFlag + "/" + tokenVehBureauNbr + "?train_nbr=" + train_nbr + "&trainType=0";
...
function Plan(dto) {
	...
	self.lev2Checked2 = ko.observable(dto.lev2Checked2);//
...
//		return self.lev1Checked() == 0 && self.needLev1Con() == '1';
	...
	self.needLev2 = ko.computed(function() {
		return (self.dailyLineFlag() == 1 && self.lev2Checked2() == 0 && self
				.needLev2Con() == 1);
	});
	// self.needLev2 = ko.computed(function() {
	// return (self.checkLev1() == 1 || self.checkLev1() == 2) &&
	// self.lev1Checked() == 1 && self.lev2Checked() == 0;
	// });
...
//		}
	...
	self.showInfoComparePanel = function() {
		console.log("进入showInfoComparePanel");
...
function TrainCfData(planTrainId) {
	var self = this;
	self.planTrainId = ko.observable(planTrainId);
}
