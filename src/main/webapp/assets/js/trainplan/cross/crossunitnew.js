...
//					self.searchModle().loadBureau(result.data); 
	...
	self.loadCrosseForPage = function(startIndex, endIndex) {   
		...
		$.ajax({
				url : "../cmcross/getCmUnitCrossInfo",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
...
//		diag.show();
	...
	/*	if(typeof(tabflag) != "undefined"){
			if(tabflag == 0){
				...
				if($('#cross_map_dlg').is(":hidden")){
					$("#cross_map_dlg").find("iframe").attr("src", "../cross/provideCrossChartData?crossId=" + unitCrossId);
...
//				if($('#cross_map_dlg').is(":hidden")){  
					$("#cross_map_dlg").find("iframe").attr("src", "../drawCross/provideCrossChartData?crossId=" 
//							+ crossId
...
//		} 
	...
	self.showTrains = function(row) {  
		...
		tabflag = 0;
		$.ajax({
			url : "../cmcross/getCmUnitCrossTrainInfo",
			cache : false,
			type : "POST",
			dataType : "json",
			contentType : "application/json",
...
function TrainRow(data) {
	...
	self.crossName = data.cmCrossName;
