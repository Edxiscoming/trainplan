...
//		console.dir(self.periodArray());
	...
	self.addModle = ko.observable(new OriginalAddCross());
...
//		console.log("trainNbr : " + self.trainNbr);
	...
	self.deleteCrosses = function(){  
		...
		showConfirmDiv("提示", "你确定要执行删除操作?", function (r) { 
	        if (r) { 
	        	commonJsScreenLock();
				$.ajax({
					url : "../originalCross/deleteOriginalCross",
					cache : false,
					type : "POST",
					dataType : "json",
					contentType : "application/json",
	...
	self.createUnitCrossInfo = function(){ 
		...
		commonJsScreenLock();
		 $.ajax({
				url : "../originalCross/transferCross",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
...
//						showWarningDialog("审核失败：" + result.message);
	...
	self.checkCrossInfo = function(){  
		...
		commonJsScreenLock();
		 $.ajax({
				url : "../originalCross/checkOriginalCross",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
...
function searchModle(){
	...
	self.createCrossFlag = ko.observable();
...
function CrossRow(data) {
	...
	self.createCrossFlag = ko.observable(data.createCrossFlag);
...
//		 return  result == "" ? "" : "相关局：&nbsp;" + result;
	...
	self.createCrossFlagStr = ko.computed(function(){
		switch (self.createCrossFlag()) {
			case "1": 
				return "<span class='label label-success'>已</span>";//"已";
				break;
			case 1: 
