...
var _stationTypeArray = [];
	...
	self.showTrainTimeCanvas = function(){
		...
		$("#run_plan_train_times_canvas_dialog2").find("iframe").attr("src", basePath+"/jbtcx/getTrainTimeCanvasPagejy?planTrainId=" + self.currentTrain().id+"&trainNbr="+self.currentTrain().name);
...
function searchModle(){
	...
	self.redundance  =ko.observable(0);
...
{ 
	...
	var endTime = new Date( data.dptTime);  //yyyy-mm-dd hhs:s
	var result = "";
