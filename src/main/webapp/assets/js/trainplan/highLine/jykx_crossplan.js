...
function heightAuto(){
	...
	$("#body_left").css("height",WH-185+"px");
	$("#left_height").css("height",WH-121+"px");
...
function CrossModel() {
	...
	self.setCurrentTrain = function(row){
		...
		self.currentTrainId(row.planTrainId);
...
//	  });
	 	commonJsScreenLock();
		$.ajax({
				url : basePath+"/highLine/getExistLinePlanByBureau",//点查询按钮执行的url
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
...
//		diag.show();
	...
	self.showCrossTrainTimeDlg = function(row){
		if(self.currentTrain() == null){
			showWarningDialog("请先选择列车");
			return;
		}
	...
	self.showRunPlans = function(){
		...
		var _height2 = $(window).height()- 195;
		if(!self.isShowRunPlans()) {//勾选
			$('#vehicle_dlg').show();
			$("#body_right").attr("style","height:500px; overflow-y: scroll; padding:15px;margin-bottom:15px;");
			$("#body_left").attr("style","height:375px;overflow-y: scroll;margin:20px 0;");		
			
		} else {//未勾选
			$('#vehicle_dlg').hide();
			$("#body_right").attr("style","height:"+_height+"px; overflow-y: scroll; padding:15px;margin-bottom:15px;");
			$("#body_left").attr("style","height:"+_height2+"px;overflow-y: scroll; margin:20px 0;");
		}
	};
