...
function heightAuto(){
	...
	//alert("window: 860---"+WH);
	$("#left_height").css("height",WH-380+"px");
	//$("#right_height").css("height",WH-51+"px");	
};
...
//			showErrorDialog("所选车号与车型不匹配");
			//权限提示：如果  所选车号与车型不匹配   不让保存
//			return false;
			showWarningDialog("所选车号与车型不匹配");
		}
		
		for(var i = 0; i < highLineCrossRows.length; i++){
			if(highLineCrossRows[i].updateFlag() == 1){//改变
					  for(var j=0;j<cdArray.length;j++){//所以车底数组
						  if(highLineCrossRows[i].vehicle1() == cdArray[j].name){
...
//			return false;
		...
		showConfirmDiv("提示", "确定要保存已修改的车底计划吗?", function (r) {
	        if (r) {
	        	commonJsScreenLock();
	        	$.ajax({
					url : basePath+"/highLine/saveHighLineWithRole/VEHICLE_SUB",
					cache : false,
					type : "POST",
					dataType : "json",
					contentType : "application/json",
...
//		self.crossAllcheckBox();  
	...
	self.init = function(){
		//显示详情
		//$('#div_hightline_planDayDetail').css({height: '560px'});//交路信息表格div高度
...
//	  });
	    ...
	    $.ajax({
			url : basePath+"/highLine/getThroughLines/VEHICLE_SUB",
			cache : false,
			type : "GET",
			dataType : "json",
			contentType : "application/json", 
	    ...
	    $.ajax({
			url : basePath+"/highLine/getDepots/VEHICLE_SUB",
			cache : false,
			type : "GET",
			dataType : "json",
			contentType : "application/json", 
	    ...
	    $.ajax({
			url : basePath+"/highLine/getAccs/VEHICLE_SUB",
			cache : false,
			type : "GET",
			dataType : "json",
			contentType : "application/json", 
	...
	self.submitHighLineWithRole = function() {
		...
		showConfirmDiv("提示", "确定要提交" + $("#runplan_input_startDate").val() + "日车底计划吗?", function (r) { 
	        if (r) {
	        	commonJsScreenLock();
	    		$.ajax({
	    			url : basePath+"/highLine/submitHighLineWithRole/VEHICLE_SUB",
	    			cache : false,
	    			type : "POST",
	    			dataType : "json",
	    			contentType : "application/json",
	...
	self.previewCmdInfo = function() {
		...
		showConfirmDiv("提示", "确定要生成" + $("#runplan_input_startDate").val() + "日日计划命令数据吗?", function (r) { 
	        if (r) {
	        	commonJsScreenLock();
	    		$.ajax({
...
self.importExcel = function(){
		...
		window.open(basePath+"/crew/highline/exportExcelcrosstianbao/1/"+_runDate+"/"+_trainNbr+"/"+_highlineFlag+"/"+searchThroughLine+"/"+searchTokenVehDepot+"/"+searchAcc+"/"+bureaus);
...
//		$.getJSON("/trainplan/assets/js/trainplan/highLine/chedi.txt", function(result){
//        	   if(result.data != null){
//					$.each(result.data,function(n, crossInfo){
...
//     });
		...
		$.ajax({
				url : basePath+"/highLine/getHighlineCrossList/VEHICLE_SUB",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
...
//							self.crossRows.loadPageRows(result.data.totalRecord, rows);
	...
	self.showRunPlans = function(){
		...
		if(!self.isShowCrossDetailInfo()) {//勾选
			//显示详情
			$("#left_height").css("height",WH-380+"px");
			//$("#div_hightline_planDayDetail").attr("style","height:560px;margin-bottom:0px;");//交路信息表格div高度
			var _height = $(window).height()-$('#div_searchForm').height()-500-150;
			_height = _height<80?200:_height;
			$('#div_crossDetailInfo').css({height:_height});//设置交路详情div高度
			$('#div_crossDetailInfo_trainStnTable').css({height: _height+67});//设置交路详情_车次详情表格div高度
			...
			//$("#div_hightline_planDayDetail").attr("style","height:745px;margin-bottom:-10px;");//设置交路详情div高度
			$("#left_height").css("height",WH-169+"px");
...
//		}
	...
	self.showTrains = function(row) {   
		...
		self.runPlanCanvasPage.clearChart();	//清除画布
		 ...
		 $.ajax({
				url : basePath+"/cross/createCrossMapVehicleBy2",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
...
//	 	    	 return false;
	...
	self.vehicle2Change = function(row){
		var _vehicle2 = (null != row.vehicle2() ? row.vehicle2() : "").toUpperCase();
		row.updateFlag(1);
		var highLineCrossRows = cross.highLineCrossRows();
		var flag1 = false;
		 var cdArray = cross.vehicleArray();//车底全部数据数组
