...
//		    		   self.nodeId(id);
		...
		_self.btnEventSaveWdCmdTrain = function() {
			...
			if(parsedJson.cmdNbrBureau==null||parsedJson.cmdNbrBureau=="" ){
				showWarningDialog("请输入路局文电号");
				return;
			}
			...
			if(parsedJson.startStn==null||parsedJson.startStn=="" ){
				showWarningDialog("请输入停运站");
				return;
			}
			if($("#input_wd_startDate").val()==null||$("#input_wd_startDate").val()=="" ){
				showWarningDialog("请选择开始日期");
				return;
			}
			if($("#input_wd_endDate").val()==null||$("#input_wd_endDate").val()=="" ){
				showWarningDialog("请选择截至日期");
				return;
			}
			if(_self.wdCmdTrain().wdCmdRuleOption().code==null||_self.wdCmdTrain().wdCmdRuleOption().code=="" ){
				showWarningDialog("请选择停运规律");
				return;
			}
