...
//			-------------是否选线判断结束
			...
			self.arrRunDaysOnblur = function(n, event){  
				...
				if(confirm("是否联动更新后序天数？")){
					//suntao 2015.6.30 
					var mul = _self.runPlanLkCMDTrainStnRows()[childIndex].arrRunDays();//dptRunDaysOld=0
				...
				if(confirm("是否联动更新后序天数？")){
					...
					_self.runPlanLkCMDTrainStnRows()[childIndex].dptRunDays(parseInt(_self.runPlanLkCMDTrainStnRows()[childIndex].dptRunDays()==null?0:_self.runPlanLkCMDTrainStnRows()[childIndex].dptRunDays()) + parseInt(mul));
					for(var i=childIndex+1; i< _self.runPlanLkCMDTrainStnRows().length; i++){
...
//					}
			...
			self.dptRunDaysOnblur = function(n, event){
				...
				if(confirm("是否联动更新后序天数？")){
					//suntao 2015.6.30 
					var mul = _self.runPlanLkCMDTrainStnRows()[childIndex].dptRunDays();//dptRunDaysOld=0
					for(var i=childIndex+1; i< _self.runPlanLkCMDTrainStnRows().length; i++){
						_self.runPlanLkCMDTrainStnRows()[i].arrRunDays(mul);
						_self.runPlanLkCMDTrainStnRows()[i].dptRunDays(mul);
					}
...
//			}
		...
		_self.importCmdTrainStnByExcel = function() {
			...
			commonJsScreenLock();
			$.ajax({
				url : basePath+"/runPlanLk/importCmdTrainStnByExcel",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
				data :JSON.stringify({
					trainNbr :objSelect.trainNbr()
				}),
				success : function(result) {
...
//					console.log("obj.createState()" + obj.createState());
		...
		self.sendMsg = function() {
			...
			$("#run_plan_sendMsg").find("iframe").attr("src", basePath+"/runPlanLk/runPlanLkAddSendMsg?cmdTrainIds="+cmdTrainIds+"&trainNbrs="+trainNbrs+"&type=0"+"&passBureaus="+passBureaus);
