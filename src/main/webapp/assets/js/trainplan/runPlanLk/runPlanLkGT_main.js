	...
	self.checkCrossInfo1 = function(row) {
		...
		if(!bol){
			...
			$.each(oneselect, function(i, n) {
				n.checkedBureau((n.checkedBureau() != null ? (n.checkedBureau() + ",") : "") + currentBureauShortName);
				// 将选中数据的,图标状态改掉.
//				n.checkFlag(1);
			});
...
//			}
	...
	self.sendMsg = function() {
		...
		var trainNbrs = "";
				...
				$("#run_plan_sendMsg").find("iframe").attr("src", basePath+"/runPlanLk/runPlanLkSendMsg?cmdTrainIds="+cmdTrainIds+"&trainNbrs="+trainNbrs+"&type=1");
	...
	self.testonecheck = function(row,event){
		...
		if (event.target.checked == true) {
			...
			$("#btn_cross_delete").attr("class", "btn btn-success disabled");
			$("#btn_cross_sendMsg").attr("class", "btn btn-success disabled");
		}
	...
	self.showRightTrains = function(row,event) {
		...
		$("#checkhis").bind("contextmenu", function(e){ return false; });
		$("#checkhis").children().children().children().attr("id",row.cmdTrainId())
...
//		var id = $("#checkhis").children().children().children()[1].planTrainId;
...
//		if($('#run_plan_train_times_edit_dialog').is(":hidden")){
			$("#run_plan_train_times_edit_dialog").find("iframe").attr("src", basePath+"/runPlan/trainRunTimePage?trainNbr="+currentTrain.trainName +
...
//		}
	...
	self.loadTrainAllPathway = function(currentTrain){	
		if(!currentTrain.isModify){
			...
			$("#run_plan_train_pathway_edit_dialog").find("iframe").attr("src", basePath+"/runPlan/pathwayPage?trainNbr="+currentTrain.trainName +
					"&trainPlanId=" + currentTrain.planTrainId+"&runDate="+moment(currentTrain.startDate).format("YYYY-MM-DD"));
			$('#run_plan_train_pathway_edit_dialog').dialog({title: "编辑列车运行经路", autoOpen: true, modal: false, draggable: true, resizable:true,
				onResize:function() {
					var iframeBox = $("#run_plan_train_pathway_edit_dialog").find("iframe");
					var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;
					var WH = $('#run_plan_train_pathway_edit_dialog').height();
					var WW = $('#run_plan_train_pathway_edit_dialog').width();
	                if (isChrome) {
	                	iframeBox.css({ "height": (WH) + "px"});
	                	iframeBox.css({ "min-height": (WH) + "px"});
	                	iframeBox.attr("width", (WW));
...
//		if($('#run_plan_train_times_edit_dialog').is(":hidden")){
		$("#run_plan_train_times_stop_dialog").find("iframe").attr("src", basePath+"/runPlan/trainRunTimeStop?trainNbr="+currentTrain.trainName+
				"&trainPlanId=" + currentTrain.planTrainId+"&runDate="+moment(currentTrain.startDate).format("YYYY-MM-DD"));
		$('#run_plan_train_times_stop_dialog').dialog({title: "开行转停运", autoOpen: true, modal: false, draggable: true, resizable:true,
			onResize:function() {
...
//		if($('#run_plan_train_times_edit_dialog').is(":hidden")){
		$("#run_plan_train_times_stop_dialog").find("iframe").attr("src", basePath+"/runPlan/trainRunTimeStop?trainNbr="+currentTrain.trainName+
				"&trainPlanId=" + currentTrain.planTrainId+"&runDate="+moment(currentTrain.startDate).format("YYYY-MM-DD")+"&type="+2);
		$('#run_plan_train_times_stop_dialog').dialog({title: "停运转开行", autoOpen: true, modal: false, draggable: true, resizable:true,
			onResize:function() {
				var iframeBox = $("#run_plan_train_times_stop_dialog").find("iframe");
				var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;
				var WH = $('#run_plan_train_times_stop_dialog').height();
...
//		if($('#run_plan_train_times_edit_dialog').is(":hidden")){
		$("#run_plan_train_times_stop_dialog").find("iframe").attr("src", basePath+"/runPlan/trainRunBakToUse?trainNbr="+currentTrain.trainName+
				"&trainPlanId=" + currentTrain.planTrainId+"&runDate="+moment(currentTrain.startDate).format("YYYY-MM-DD") + "&type=2");
		$('#run_plan_train_times_stop_dialog').dialog({title: "备用转开行", autoOpen: true, modal: false, draggable: true, resizable:true,
			onResize:function() {
				var iframeBox = $("#run_plan_train_times_stop_dialog").find("iframe");
				var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;
				var WH = $('#run_plan_train_times_stop_dialog').height();
...
//		console.dir(checkflagchoice)
		...
		$.ajax({
				...
				data :JSON.stringify({
					...
					tokenVehBureau : $('#bureausSel  option:selected').text(),//担当局
	...
	self.showRunPlans = function(){
		...
		if(self.isShowRunPlans()) {//未勾选
			...
			/*$("#plan_cross_default_panel").attr("style","height: 710px; margin-top: 10px;margin-bottom: -15px;");
			$('#row_table').css({height: '620px'});
			$('#canvas_parent_div').css({height: '725px'});*/
			 ...
			 $('#left_height').css("height",WH-490+"px");
			 $('#canvas_parent_div').css("height",WH-380+"px");
			/* $("#plan_cross_default_panel").attr("style","height: 455px;margin-top: 10px;margin-bottom: 0px;");
			 $('#row_table').css({height: '530px'});
			 $('#canvas_parent_div').css({height:'470px'});*/
		}
				 ...
				 //通过cmdtrainid  查询详细信息 展示
					...
					success : function(result) {
						...
						if (result != null && result != "undefind" && result.code == "0") { 
								...
								self.searchModle().cmdItem(result.data.cmdItem)
								self.searchModle().cmdNbrSuperior(result.data.cmdNbrSuperior)
								self.searchModle().cmdTime(result.data.cmdTime)
								self.searchModle().largeContent(result.data.largeContent)
								self.searchModle().releasePeople(result.data.releasePeople)
...
function ModifyRecord(data){
	...
	//self.planCheckId  =ko.observable(data.planCheckId ) ;
	 ...
	 self.startDate           =ko.observable(moment(filterValue(data.startDate)).format("YYYYMMDD")) ;
	 self.endDate           =ko.observable(moment(filterValue(data.endDate)).format("YYYYMMDD")) ;
	 
	 self.selectedDate        =ko.observable(data.selectedDate ) ;
	 self.modifyContent       =ko.observable(data.modifyContent   ) ;
	 self.modifyTime          =ko.observable(moment(filterValue(data.modifyTime)).format("MMDD HH:mm")) ;//MMDD HH:mm
...
//				console.log("---"+ data.passBureau.substring(i, i+1));
	...
	self.checkCss = ko.computed(function(){ 
		if(self.checkFlag() != 2 && data.passBureau != null && data.passBureau.indexOf(currentBureauShortName) > -1){
			//和当前局相关并且被当前局审核过了的
			if(self.checkedBureau() != null){
				...
				}else if(self.checkedBureau().indexOf(currentBureauShortName) > -1){
		...
		}else if(self.checkFlag() != 2 && (data.passBureau == null || (data.passBureau != null && data.passBureau.indexOf(currentBureauShortName) < 0))){
			//和当前局无关 未被完全审核的
			return "fa fa-pencil-square-o gray";
		}else if(self.checkFlag() == 2){
			if(data.passBureau == null || (data.passBureau != null
						&& data.passBureau.indexOf(currentBureauShortName) < 0)){
...
function RunPlanRow(data){
	...
	if(data.isModified==0 || data.isModified=="0"){
		...
		self.planTrainId = data.planTrainId;
		str ='<span class="label label-success"><i class="fa fa-book triangle"></i><a href="javascript:void(0)" onclick="showOneModifyRecord(\''+self.planTrainId+'\')" class="span_text">开</a></span>';
	}
