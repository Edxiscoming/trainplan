...
//						console.log('name='+name);
						return name;//这个name就是显示到input框中的值
					}
...
//			-------------是否选线判断结束
			...
			self.arrRunDaysOnblur = function(n, event){
				...
				if(confirm("是否联动更新后序天数？")){
					//suntao 2015.6.30 
					var mul =0;
//					var arrRunDaysOld = runPlanLkCMDTrainStnArray[childIndex].arrRunDays();
//					if(arrRunDaysOld!=null){
//						mul= _self.runPlanLkCMDTrainStnRows()[childIndex].arrRunDays() - arrRunDaysOld;
//					}
//					else{
						mul= _self.runPlanLkCMDTrainStnRows()[childIndex].arrRunDays();//dptRunDaysOld=0
//					}
					_self.runPlanLkCMDTrainStnRows()[childIndex].dptRunDays(mul);
					for(var i=childIndex+1; i< _self.runPlanLkCMDTrainStnRows().length; i++){
						_self.runPlanLkCMDTrainStnRows()[i].arrRunDays(mul);
						_self.runPlanLkCMDTrainStnRows()[i].dptRunDays(mul);
//						_self.runPlanLkCMDTrainStnRows()[i].arrRunDays(parseInt(_self.runPlanLkCMDTrainStnRows()[i].arrRunDays()==null?0:_self.runPlanLkCMDTrainStnRows()[i].arrRunDays()) + parseInt(mul));
//						_self.runPlanLkCMDTrainStnRows()[i].dptRunDays(parseInt(_self.runPlanLkCMDTrainStnRows()[i].dptRunDays()==null?0:_self.runPlanLkCMDTrainStnRows()[i].dptRunDays()) + parseInt(mul));
					}
				 }
			...
			self.dptRunDaysOnblur = function(n, event){
				...
				var days = self.dptRunDays();
				 ...
				 if(confirm("是否联动更新后序天数？")){
					//suntao 2015.6.30 
					var mul =0;
//					var dptRunDaysOld = runPlanLkCMDTrainStnArray[childIndex].dptRunDays();
//					if(dptRunDaysOld!=null){
//						mul= _self.runPlanLkCMDTrainStnRows()[childIndex].dptRunDays() - dptRunDaysOld;
//					}
//					else{
						mul= _self.runPlanLkCMDTrainStnRows()[childIndex].dptRunDays();//dptRunDaysOld=0
//					}
					for(var i=childIndex+1; i< _self.runPlanLkCMDTrainStnRows().length; i++){
						_self.runPlanLkCMDTrainStnRows()[i].arrRunDays(mul);
						_self.runPlanLkCMDTrainStnRows()[i].dptRunDays(mul);
...
//									}
//								
//								}else if(childIndex == i && childIndex!=len-1){//中间的行
//									var ds =0;
...
//									}
//								
//								}else if(childIndex == i && childIndex ==len-1 && len >1){//最后一行
//									var ds =0;
//									if(obj.arrRunDays()!='--' && upobj.dptRunDays()!='--'){
//										ds = obj.arrRunDays()-upobj.dptRunDays();//A站的出发天数-A站的到达天数>=1
//									}
...
//									
//								
//								}else if(childIndex == i && childIndex ==len-1 ){//最后一行
//									var ds =0;
//									if(obj.arrRunDays()!='--' && uobj.dptRunDays()!='--'){
//										ds = obj.arrRunDays()-uobj.dptRunDays();//A站的出发天数-A站的到达天数>=1
//									}
...
//				obj.currentBureauJc=$("#"+z+"jy100").find("option:selected").text();
				var strBur = $("#"+z+"gt100").find("option:selected").text();//获取页面下拉框的值
...
//			if($('#run_plan_train_times_edit_dialog').is(":hidden")){
				$("#his_traininfo_dialog").find("iframe").attr("src", basePath+"/runPlanLk/hisTrainInfoPageGT?tabType=bjdd&trainNbr="+_selectCmdTrainArray[0].trainNbr()+"&cmdTrainId="+_selectCmdTrainArray[0].cmdTrainId());
...
//			}
		...
		_self.importCmdTrainStnByExcel = function() {
			...
			if (1 == objSelect.createState()) {
				showWarningDialog("您所选中的命令中含有已经生成开行计划的命令，不能导入文件时刻");
				return;
			}
...
//		    var addFlag = self.searchModle().upLoadFlag();
	        $.ajaxFileUpload
	        ({
                url:'importCmdTrainStnByExcel',
                secureuri:false,
                fileElementId:'fileToUpload',
                type : "POST",
                dataType: 'json',  
                data:{
                	trainNbr:_self.currentCmdTxtMl().trainNbr(),
                	startStn:_self.currentCmdTxtMl().startStn(),
                	endStn:_self.currentCmdTxtMl().endStn(),
                },
                success: function (data, status)
                {  
                	
                	if (data != null && typeof data == "object" && data.code == "0") {
						if(data.data != null){
...
//								}
		...
		_self.findCmdTrainStn = function() {
			var _selectCmdTrainArray = [];
			var num = 0;
			var i;
			var obj = null;
			...
			commonJsScreenLock();
			$.ajax({
				url : basePath+"/runPlanLk/findCmdTrainStn",
				cache : false,
				type : "POST",
				dataType : "json",
				contentType : "application/json",
				data :JSON.stringify({
					cmdTxtMlItemId :objSelect.cmdTxtMlItemId()
				}),
				success : function(result) {
...
//				}
				var z=n+1;
//				obj.stnName=$("#"+z+"gt").val();
//				obj.stnNameTemp=$("#"+z+"gt").val();
//				obj.currentBureauJc=$("#"+z+"gt100").find("option:selected").text();
				var strBur = $("#"+z+"gt100").find("option:selected").text();//获取页面下拉框的值
				if(obj.nodeName()=="" || obj.nodeName()==undefined){
					showErrorDialog("站名不能为空");
					isNull = true;
...
//			}
		...
		_self.saveOtherCmdTxtMl = function() {
			...
			$.each(_self.runPlanLkCMDTrainStnRows(),function(n, obj){
				var z=n+1;
				obj.stnName=$("#"+z+"wjgt").val();
				obj.stnNameTemp=$("#"+z+"wjgt").val();
				obj.currentBureauJc=$("#"+z+"wjgt100").find("option:selected").text();
				var strBur = $("#"+z+"gt100").find("option:selected").text();//获取页面下拉框的值
				obj.stnBureau=strBur;
				if(obj.nodeName()=="" || obj.nodeName()==undefined){
					showErrorDialog("站名不能为空");
...
//						loadDataForWjddPage();
		...
		_self.batchCreateRunPlanLine = function() {
			...
			for(var i=0;i<_self.runPlanLkCMDRows().length;i++){
				...
				if (obj.isSelect() == 1) {
//					console.log("passBureau: " + obj.passBureau());
//					if (obj.isExsitStn() == "0") {
...
//			}
			...
			_self.runPlanLkCMDTrainStnRows.splice(_self.currentCmdTrainStn().childIndex()+1, 0, new _self.cmdTrainStnTimeRow({
				...
				nodeId:_self.currentCmdTrainStn().nodeId(),
				jobs:"",
				//nodeName:_self.currentCmdTrainStn().nodeName(),
				nodeName:"",
				bureauId:"",
				nodeStationId : "",
				nodeTdcsId  : "",
		...
		_self.cmdTypeWjddChangeEvent = function() {
			...
			if (_self.searchModle().cmdTypeOptionWjdd().code == "1"
				...
				$("#div_wjdd_cmdTrainTable_jk").show();//屏蔽外局担当 既有、高铁加开table div
				$("#div_wjdd_cmdTrainTable_wd").hide();//屏蔽外局担当文电加开table div
			}
		};
...
//			$("#input_wd_endDate").datepicker();//文电终止日期输入框
		...
		self.sendMsg = function() {
			...
			for(var i=0;i<_self.runPlanLkCMDRows().length;i++){
				var obj = _self.runPlanLkCMDRows()[i];
				if (obj.isSelect() == 1) {
					cmdTrainIds.push(obj.cmdTrainId());
					if(null != obj.trainNbr()){
						trainNbrs += (trainNbrs == "" ? "" : ",");
						trainNbrs += obj.trainNbr();
					}
					if(null != obj.passBureau()){
						passBureaus += (passBureaus == "" ? "" : ",");
						passBureaus += obj.passBureau();
					}
				}
			}
			
			
			if(cmdTrainIds.length == 0) {
				showWarningDialog("请选择数据！");
				return;
			}else if(cmdTrainIds.length > 1){
				showWarningDialog("只能选择1条数据！");
				return;
			}
			
			$("#run_plan_sendMsg").find("iframe").attr("src", basePath+"/runPlanLk/runPlanLkAddSendMsg?cmdTrainIds="+cmdTrainIds+"&trainNbrs="+trainNbrs+"&type=1"+"&passBureaus="+passBureaus);
...
//			var _height2 = $(window).height()- 236;
			if(!self.isShowRunPlans()) {//勾选
				$('#time_div').show();
				$("#time_div2").show();
				$("#left_div").attr("style","margin-right:-590px; float:left; width:100%;");
				$("#left_div_panel").attr("style","margin-right:590px;");
				$("#time_div").attr("style","width:580px;");
				$("#right_div").attr("style","margin-right:-590px; float:left; width:100%;");
				$("#right_div_panel").attr("style","margin-right:590px;");
				$("#time_div2").attr("style","width:580px;");
				
			} else {//未勾选
				$('#time_div').hide();
				$("#left_div").attr("style","margin-right:0px; float:left; width:100%;");
...
function heightAuto(){
	...
	$("#time1").css("height",WH-251+"px");
	$("#left_height2").css("height",WH-220+"px");
	$("#time2").css("height",WH-251+"px");
