 ...
 */
public class PlanLineSTNDTOCOM {
    // 记录id
    private String id;
    ...
    private String arrTime_hm;
    private String dptTime_hm;
    ...
    public PlanLineSTNDTOCOM(){}
    public PlanLineSTNDTOCOM(Map<String, Object> map) {
    ...
    public PlanLineSTNDTOCOM(Map<String, Object> map, String bureau) {
        this.index = MapUtils.getIntValue(map, "STN_INDEX");
        this.stnName = MapUtils.getString(map, "NODE_NAME");
        this.bureau = MapUtils.getString(map, "BUREAU");
        this.stnBureau = MapUtils.getString(map, "STNBUREAU");
        this.trackName = MapUtils.getString(map, "TRACK_NAME");
        // 0101 01:01
        this.arrTime = MapUtils.getString(map, "ARR_TIME", "");
        if(this.arrTime.length() > 0) {
            this.arrTime = this.arrTime.substring(5, 16).replace("-", "");
        }
        this.dptTime = MapUtils.getString(map, "DPT_TIME", "");
        if(this.dptTime.length() > 0) {
            this.dptTime = this.dptTime.substring(5, 16).replace("-", "");
        }
        // 01:01
        this.arrTime_hm = this.arrTime.substring(5,this.arrTime.length());
        this.dptTime_hm = this.dptTime.substring(5,this.arrTime.length());
        
        this.owner = MapUtils.getString(map, "STN_BUREAU", "").equals(bureau);
    }

    public PlanLineSTNDTOCOM(Map<String, Object> map, String bureau, String id) {
        this.id = id;
        this.index = MapUtils.getIntValue(map, "STN_INDEX");
        this.stnName = MapUtils.getString(map, "STN_NAME");
        this.bureau = MapUtils.getString(map, "BUREAU");
	...
	public String getArrTime_hm() {
		return arrTime_hm;
	}
	public void setArrTime_hm(String arrTime_hm) {
		this.arrTime_hm = arrTime_hm;
	}
	public String getDptTime_hm() {
		return dptTime_hm;
	}
	public void setDptTime_hm(String dptTime_hm) {
		this.dptTime_hm = dptTime_hm;
	}
}