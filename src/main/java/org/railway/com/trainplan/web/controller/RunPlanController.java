...
public class RunPlanController {
	...
	private String dealCrossStartDate(String startDate){
		String str = "";
		if(startDate != null && !"".equals(startDate.trim())){
			boolean flag = true;
			String [] dateAry = startDate.split("-");
			String temp = dateAry[0];
			for(String date : dateAry){
				if(!temp.equals(date)){
					flag = false;
					break;
				}
		...
		try {
			...
			if (!rowList.isEmpty()) {
				for (int j = 0; j < rowList.size(); j++) {
					...
					String startDate = dealCrossStartDate(crossRunPlanInfo.getCross_start_date());
					crossRunPlanInfo.setCross_start_date(startDate);
