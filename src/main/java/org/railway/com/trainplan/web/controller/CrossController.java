...
public class CrossController {
	...
	@ResponseBody
	@RequestMapping(value = "/createCrossAlternateMap", method = RequestMethod.POST)
	public Result createCrossAlternateMap(
		...
		PlanCrossInfo new_planCross = runPlanService.findById(planCrossId);
		if(new_planCross != null ){
			//逻辑交路信息
			TCmCross cross = cmCrossServiceImpl.getTCmCross(new_planCross.getUnitCrossId());
			//原始对数表信息
			TCmOriginalCross ocross = cmOriginalCrossServiceImpl.getTCmOriginalCross(cross.getCmOriginalCrossId());
			//交替前的方案
			TCmVersion pre_version = tCmVersionServiceImpl.getPreVersion(new_planCross.getBaseChartId());
			/*重新计算起始结束日期*/
			String startTime = operationlanServiceImpl.selectFirstRunDatePlanTrain(planCrossId).getRunDate();
			startTime = TimeUtils.date2String(TimeUtils.string2Timestamp(TimeUtils.YYYYMMDD, startTime), TimeUtils.YYYY_MM_DD_FORMAT);
			...
			if(pre_version != null){
				...
				param.put("mTemplateScheme", pre_version.getmTemplateScheme());
				TCmCross pre_cross = cmCrossServiceImpl.selectByCrossNameAndScheme(param);
				if(pre_cross != null){
					PlanCrossInfo pre_planCross = runPlanService.findByUnitCrossId(pre_cross.getCmCrossId());
					if(pre_planCross != null){
						has_pre = true;
						String pre_endTime = startTime;
						//源计划 往前查10天的数据
						String pre_startTime = TimeUtils.getNextDateString(pre_endTime, -10, TimeUtils.YYYY_MM_DD_FORMAT);
						pre_endTime = TimeUtils.getNextDateString(pre_endTime, 10, TimeUtils.YYYY_MM_DD_FORMAT);
						
						reqMap.put("startTime", pre_startTime);
						reqMap.put("endTime", pre_endTime);
						reqMap.put("planCrossId", pre_planCross.getPlanCrossId());
						Result preCrossResult = provideTrainLineChartDate(reqMap);
						
						Map<String, Object> preMap = (Map<String, Object>)preCrossResult.getData(); 
						dataMap.put("myJlData_pre", preMap.get("myJlData"));
						dataMap.put("gridData_pre", preMap.get("gridData"));
					}
				}
			}
			String new_startTime = startTime;
			//新计划 往后查20天的数据
			int days = 20;
			if(!has_pre){
				days = 30;
			}
			String new_endTime = TimeUtils.getNextDateString(new_startTime, days, TimeUtils.YYYY_MM_DD_FORMAT);;
			reqMap.put("planCrossId", planCrossId);
			reqMap.put("startTime", new_startTime);
			reqMap.put("endTime", new_endTime);
			Result newCrossResult = provideTrainLineChartDate(reqMap);
			Map<String, Object> newMap = (Map<String, Object>)newCrossResult.getData(); 
