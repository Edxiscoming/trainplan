...
//				cmCrossServiceImpl.addTCmTrain(train);
			...
			ocross.setCheckPeopleOrg(user.getBureau());
...
//				tCmTrainRuleMapper.insertSelective(rule);
			...
			sortCmTRainList(trainlist, alternatemap);
...
//		List<TCmTrain> cmTrains = cmCrossServiceImpl.getTCmTrain(cross);
		...
		for(TCmTrain train : cmTrains){
			...
			for(int cirNum=0;cirNum < ocross.getGroupTotalNbr(); cirNum++){//循环
				...
				if(train.getTrainSort() == 1 && cirNum < ocross.getGroupTotalNbr()){
					//第一个车次，给出组数信息
		...
		while(it.hasNext()){
			...
			for (TCmPhyCrossTrain tCmPhyCrossTrain : lt) {
				...
				for(TCmPhyCross phycross : phyCrossList){
					if(tCmPhyCrossTrain.getCmPhyCrossId().equals(phycross.getCmPhyCrossId())){
						b.setGroupSn(phycross.getGroupSn()+"");
					}
				}
...
//					t/1000/60
	...
	/*按交替日期 将逻辑交路车次重新排序，为生成车底交路使用*/
	private void sortCmTRainList(List<TCmTrain> trainlist, final Map<String,TCmTrainAlternate> alternatemap){
		/*按交替日期对交路中的车次进行排序*/
		Collections.sort(trainlist,new Comparator<TCmTrain>() {
			...
			public int compare(TCmTrain o1, TCmTrain o2) {
				...
				if(alternatemap != null){
					o1_Date = alternatemap.get(o1.getCmTrainId()).getAlternateDate();
					o2_Date = alternatemap.get(o2.getCmTrainId()).getAlternateDate();
				} else {
					//车次的交替信息
					o1_Date = cmCrossServiceImpl.getTCmTrainAlternate(o1.getCmTrainId()).getAlternateDate();
					o2_Date = cmCrossServiceImpl.getTCmTrainAlternate(o2.getCmTrainId()).getAlternateDate();
				}
				Date o1Date = TimeUtils.string2Timestamp(TimeUtils.YYYYMMDD, o1_Date);
				Date o2Date = TimeUtils.string2Timestamp(TimeUtils.YYYYMMDD, o2_Date);
				if(o1Date.getTime() > o2Date.getTime()){
					return 1;
				}
				if(o1Date.getTime() == o2Date.getTime()){
		...
		for(int i = 0 ;i< trainlist.size();i++){
			if(temp_train_sort != 0){
				temp_train_sort = CrossInfoGenHelper.getNextTrainSort(temp_train_sort, trainlist.size());
		...
		for(int i=trainlist.size()-1 ; i>=0;i--){
			if(temp_train_sort != 0){
				temp_train_sort = CrossInfoGenHelper.getPreTrainSort(temp_train_sort, trainlist.size());
				trainlist.get(i).setTrainSort(temp_train_sort);
			}
			if(trainlist.get(i).getCmTrainId().equals(firstTrain.getCmTrainId())){
				trainlist.get(i).setTrainSort((short)1);
